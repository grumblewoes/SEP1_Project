...
public class BoardGamesModelManager implements BoardGamesModel, Serializable
{
   ...
   *
   * @param wish wish that is to be voted for
   * @see BoardGamesFileManager
   */
   ...
   *
   * @param wish wish that is to be removed
   * @see BoardGamesFileManager
   */
  public void voteForWish(Wish wish)
  {
    wishList.voteForWish(wish);
    fileManager.exportModelToDatabase();
  }
   ...
   *
   * @param title the string value by which the wish is to be found
  ...
  {
    return wishList.getVotesForWish(title);
  }
  ...
  {
    return wishList.getWishByTitle(title);
  }
   ...
   *
   * @param title the string value by which the game is to be found
   * @return game
   * the Game instance that matches the query or null
   */
  public Game getGameByTitle(String title)
  {
    return gameList.getGameByTitle(title);
  }
  ...
  {
    reservationList.removeExpiredReservation();
    fileManager.exportModelToDatabase();
  }
   ...
   *
   * @param game the game that is to be checked if is reserved
   * @return boolean
   * true or false that indicates if the games is reserved
   */
  public boolean isReserved(Game game)
  {
    return reservationList.isReserved(game);
  }
   ...
   *
   * @param associate a new associate that is to be added to the clubAssociateList
   */
  public void addClubAssociate(ClubAssociate associate)
  {
    clubAssociateList.addClubAssociate(associate);
    fileManager.exportModelToDatabase();
  }
   ...
   *
   * @param id the unique id of the associate whose status is to be toggled
   */
  public void updateClubAssociate(int id)
  {
    clubAssociateList.updateClubAssociate(id);
    fileManager.exportModelToDatabase();
  }
  ...
  {
    return clubAssociateList.getAllClubAssociates();
  }

  /**
   * A method that calls eventList to add the and calls the fileManager to save the model.
   *
   * @param event the event that is to be added
   * @see BoardGamesFileManager
   */
   ...
   *
   * @param title the unique title of the event that is to be removed
   * @see BoardGamesFileManager
   */
  public void removeEvent(String title)
  {
    eventList.removeEvent(title);
    fileManager.exportModelToDatabase();
  }
  ...
  {
    return eventList.getNumberOfEvents();
  }
